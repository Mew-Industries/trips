...
# Keeps the climate curve sensible before exact dates are locked. Every stop in
# this itinerary already has concrete dates, so this stays empty for now.
PENDING_FALLBACK: dict[str, tuple[int, int]] = {}


def _get(url: str, timeout: int = 25) -> dict:
    req = urllib.request.Request(url, headers={"User-Agent": "usa-roadtrip-weather/1.0"})

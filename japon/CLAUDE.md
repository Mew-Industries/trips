...
4. **Días** — las 44 jornadas del primer despegue al último aterrizaje: dónde estás, dónde dormís, el **plan fijo** del día y las **sugerencias** como lista. Cada jornada tiene además su vista individual a pantalla completa con URL propia (`?jornada=<fecha>`, ver su sección).
...
Ruteo por URL: `?tab=hospedajes|transportes|dias` (sin parámetro = resumen), con `pushState`, así que back/forward andan y el link es compartible. Un `?tab=` desconocido cae en resumen. `?dia=`, `?tramo=`, `?hosp=` y `?jornada=` valen por su tab cuando no hay `tab=` explícito; por eso, si alguno está puesto, tocar "Resumen" en el riel **escribe** `tab=resumen` en vez de borrar el parámetro (si no, el implícito te devolvía a la vista de la que querías salir).

## La vista de día: una jornada a pantalla completa (2026-09-17 · task 660)

Martín, 17/9 con el screenshot de la tarjeta del 19/10: «cada día debería tener una vista individual full screen para abrir/compartir. Tanto esa vista como la vista de la tab Días debería mostrar el full itinerario fijo (cosas confirmadas como transporte, reservas, etc.) junto con todos los detalles de logística necesarios, y una lista de sugerencias de qué hacer ese día».

- **`?jornada=<fecha>` es la vista de día**, una capa `position: fixed` por ENCIMA de todo (header, mapa, riel) que se crea al montar y se llena al abrirse. No es una tab más: cerrarla devuelve el site tal cual estaba, con Días abajo. El estado vive en la URL como el resto (`?dia=`/`?tramo=`/`?hosp=`), así que el link es compartible, el back la cierra y Esc también. `#/dia/<fecha>` —la forma que se escribe a mano— se normaliza a la canónica con `replaceState` y no deja entrada en el histórico. Una fecha que no es del viaje se ignora, igual que un `?tab=` desconocido.
- **Las dos mitades del día están en las DOS vistas** (la tarjeta de la tab y la de pantalla completa), armadas por las mismas funciones de `views.js` — si cambia una, cambian las dos:
  1. **Plan fijo** (`fixedPlanHtml`): los `day.events` que ya arma `itinerary.js`, con la logística que **ya estaba en los datos** pegada a cada uno — modo y duración del tramo, terminal de salida y de llegada (`leg.fromTerminal`/`toTerminal`), `leg.detail`, "cómo llegar" (`leg.dirUrl` o unas directions de Maps), la ventana de check-in/out vía el mismo `hoursParts()` de Hospedajes, el número de reserva y los links del alojamiento, y para los vuelos el horario del segmento, el avión y el tracker.
...
- **Pasar de día: los botones y el teclado cuentan igual** (task 687). En la vista de un día, `→`/`←` van al siguiente/anterior — la misma cuenta que los botones ‹ › de la barra (`neighborDay()`, que **no envuelve**: el 6/10 no tiene anterior ni el 18/11 siguiente) y la misma navegación (`goJornada()`, o sea la URL y el histórico). Las teclas de la vista (Esc incluida) viven en **un solo** `keydown` de `views.js`: dos listeners para las mismas teclas se pisan. La `d` del modo discreto sigue siendo del mapa, en `index.html`. Las flechas siguen su camino normal si hay foco de texto (input/textarea/select/contenteditable), un arrastre en curso (`body.plan-dragging`), el lightbox abierto (usa ‹ › para las fotos) o la tecla viene con modificador / ya la atendió alguien.
- Verificación: `node evidence/660/tooling/check.mjs <base> <shots>` en el workspace de Mew (44 checks: ruteo, el 19/10 completo, sugerencias como lista, bordes del viaje, meta/preview, mobile, los 11 legs sin hora, el resto del site y la compartida) + `node japon/scripts/check_dia_view.js` (invariantes sobre el modelo, sin navegador) + `node japon/scripts/check_dia_keys.mjs <shots> <base>` (27 checks del teclado en Chromium: flechas, bordes, guardas y la `d`).
...
- **El margen del check-in tiene signo y el reloj da la vuelta.** `checkInMargin` es la distancia entre la llegada y `checkInTo`, y las dos puntas del rango son informativas: si da negativo la línea dice «llegás 40 min tarde» (nunca un `-1 h -40`, que es lo que salía de restar en crudo), y si la ventana cruza la medianoche —Osaka abre 14:00 y cierra **01:00**, o sea 01:00 del día siguiente— `clockDiff` suma las 24 h. Lo que NO se corrige es el signo: llegar 20:10 a un límite de 20:00 tiene que seguir dando −10. El disparador es `checkInTo < checkInFrom`, que ya está en el dato; no hace falta un campo nuevo.
...
- **El site tiene UN tema.** No hay dark mode (`prefers-color-scheme` no aparece en ningún lado), así que pedirle `colorScheme: 'dark'` a Playwright devuelve la misma imagen: un antes/después "en los dos temas" sale en cuatro capturas idénticas, que no prueban nada. Los dos ejes que sí existen son el **modo discreto** (🙈 / tecla `d` — ahí la card del itinerario está oculta a propósito, que es lo que el modo enmascara) y los **dos anchos** (1400 / 390 px).
...
**Familias de capas.** El control de arriba a la derecha tiene cuatro: `destinos` · `actividades` (guardados de Maps + day trips + reels + huérfanos: para quien mira el mapa son la misma cosa, y los chips de categoría cortan más fino adentro) · `hospedaje` · `transporte`, más un master **"Todo"** que las apaga o prende de una. Sin estado en la URL el mapa aterriza **solo con `actividades`** (`DEFAULT_LAYERS_OFF = 'destinos,hospedaje,transporte'`, pedido del 2026-09-19; "Todo" queda sin marcar). El estado va en `?layersOff=transporte` y un `layersOff` presente —aunque venga vacío, que es "todo prendido"— manda sobre el default. `revealLayer()` prende la familia cuando se va a un punto desde una lista, una ficha o un deep-link (`?hosp=`) y su capa estaba apagada — con hospedajes apagados por default ese es ahora el camino normal, no el borde.
...
- **Estructura**: **40 noches**, **17 nodos numerados** (15 destinos + 2 nodos "de paso"). Tokio está PARTIDO: 5 noches al llegar (parada 1, 8–13 oct) + 3 noches al final pre-vuelo (parada 17, 14–17 nov, última noche 16/11; el 17 es día final pre-vuelo de la tarde). El viaje termina de vuelta en Tokio para el vuelo del 17/11 (llega BA 18/11). **Chain norte al arranque** (sin backtrack): Tokio → Nikko (de paso) → **Ichinoseki 1n** (base; foco Geibikei; Hiraizumi/Chūson-ji opcional a 8 min) → **Sendai 1n** → Sapporo 2n → vuelo a Komatsu → Kanazawa. Hokkaido en oct está templado (~17°/7°); el norte de Honshu (Iwate/Yamagata) en foliage arrancando. **Sin Hiroshima**. **Medio re-armado el 25/8 (task 545)**: Kioto 5n · Osaka 3n · **Tokio del medio 4n** (parada 10, con Zava y Ari). **Cierre re-armado el 3/9 (tasks 585+586) y ajustado el 6/9 (task 607, decisión de Martín)**: plan B — Yoshiike no tenía lugar 12-14, así que la reserva 10-12 se mantiene y el orden del plan A se invirtió; el 6/9 la noche en Naoshima se reemplazó por una noche de momiji en Kioto (Naoshima pasó a day trip: primero del sáb 24/10, después del vie 23/10 y, desde el 15/9 12:40 con las entradas recompradas ×4, del **jue 22/10** ida y vuelta desde Kioto — vive en `daytrips` de la parada 8, y Chichu 12:45 / Benesse ~14:30 / Minamidera 15:20 son `activities` con `at` de ese nodo; el 23 quedó como día completo en Kioto y el 24 como traslado simple Kioto → Osaka). De Seúl se vuela a **HND** y siguen **Hakone 10-12 (2n, Yoshiike sin cambios)** → **Kioto momiji 12-13 (1n, hospedaje por reservar)** → **Koyasan 13-14 (1n, Eko-in reservado)** → Tokio final 14-17 (3n).

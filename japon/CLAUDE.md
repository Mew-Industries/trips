...
- **Una reserva que no es de Martín va SIN `booking`.** El bloque se titula "Mi reserva en …" y linkea a *su* cuenta: ponerlo cuando reservó otro de los cuatro dice algo falso y publica un código que él no puede abrir. En ese caso el hospedaje se carga entero (nombre, zona, fotos, `url`, `mapsUrl`) y las horas van en `checkInFrom`/`checkOutBy`, que es lo que `hoursHtml()` muestra cuando no hay reserva. Así está Kioto (task 553).
...
sin cancelarlo: la reserva del shukubo sigue viva y el candidato es 10-11 nov (post-Seúl),
pero la decisión no está tomada. Para eso existe el tercer tipo de nodo:
...
- **`type: 'pendiente'`** = la parada existe (ficha, fotos, hospedaje, pin en el mapa) pero
  **no ocupa ninguna noche del viaje**. Va con `n: null`, `start: null`, `end: null`,
  ...
  completo que se lee en la tarjeta. La suma de noches y la numeración 1..15 son de la
...
- **Hospedaje de Kioto** (25 ago 2026 · task 553): el Airbnb propio de Ayakōji-dōri (19-25, para 2) **quedó reemplazado** por el que sacó otro de los cuatro: **«Kyotofish · Kamogawa»**, machiya entera de 180 m² sobre el río en Shimogyō-ku (348-2 Yatsuyanagichō, 600-8112), 4 dormitorios / 8 camas / 3 baños, para los 4, **19-23 oct** (check-in 16:00, checkout 10:00). Va **sin `booking`** (no es la reserva de Martín, ver arriba). Ojo: **la reserva cubre 4 noches y el nodo son 5** (19-24) — la noche del 23 está sin resolver y el `lodging.pending` lo dice en la tarjeta; **las fechas del nodo NO se tocaron**, eso lo decide Martín. Coords de Google para la dirección: `34.9940, 135.7671`.

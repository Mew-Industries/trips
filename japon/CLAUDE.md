...
- **El orden de la línea es el del listado, y sale del listado.** `dayRoute(day, ctx)` en `views.js` arma el recorrido con `catGroups()` —la función que ordena la lista, extraída de `catListHtml()` justamente para que no puedan divergir—: primero lo que tiene hora comprada (`activity.at`, ya cronológico), después las sugerencias en orden de lista, y la cama al final. Lo que no tiene `coords` no entra en la línea pero sigue en la lista. Cada punto lleva su ordinal (`.rt-ord`) y el mismo número aparece en su ítem (`.sg-item[data-ord]`, puesto por `showDay()`): numerar en el mapa algo que no se puede volver a encontrar en la lista no sirve de nada.
...
**Reversibilidad**: las vistas son 4 commits de la 499 (shell, hospedajes, transportes, días) sobre 2 de modelo, y 4 de la 500 (mapa fuera de las tabs, Días por categorías, catálogo completo, fechas en discreto). La 508 son 2 más, independientes entre sí: primero las capas, después el foco de día (que sí depende del primero — usa `TOGGLES`/`layerOn()` para apagar y restaurar). `git revert` de cualquiera de los rangos deja el árbol idéntico al del commit anterior y el site andando — probado en los tres. Revertir las vistas **no** toca el modelo extendido (`start`/`end`, horarios, `why`), que sirve por sí solo.

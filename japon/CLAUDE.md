...
**Fotos y links del `lodging`** (2026-08-07 · task 491): además de `img` (miniatura), el objeto acepta `imgs: [...]` — galería del alojamiento; la 1ª es la miniatura de la tarjeta y el resto va en una tira debajo (`.lodging-gallery`, mismo estilo que la galería del destino) que abre el lightbox. `url` es **la ficha pública del alojamiento** (Airbnb / Rakuten `hotel.travel.rakuten.co.jp/hotelinfo/plan/<id>` / Booking `.en-gb.html` / sitio oficial) y `mapsUrl` **siempre** el link a Maps: van como links separados y `lodgingLinkLabel()` rotula el primero según el dominio. Nunca URL de Booking con token de sesión, y nada de PIN/credenciales de la reserva (el site es público). Las fotos de hoteles no-Airbnb se bajan y se sirven **locales** desde `img/<slug>-<qué>.jpg` (máx 900 px, JPEG q80) — los CDN de Rakuten/Booking no son hotlinkables de forma confiable; el de Airbnb (muscache) sí, y por eso esos listings siguen con URL remota.

#!/usr/bin/env node
/**
 * Genera `<viaje>/dia/<fecha>.html`: una página estática por jornada, con los meta de
 * Open Graph de ESE día, que redirige a la app (task 660).
 *
 *   node <viaje>/scripts/build_dias.js            # escribe
 *   node <viaje>/scripts/build_dias.js --check    # no escribe; sale 1 si quedaron viejas
 *
...
// La carpeta del viaje se deriva de la ruta del script: el mismo archivo sirve
// para cualquier viaje del monorepo sin tocar nada.
const SITE = 'https://mew-industries.github.io/trips/' + path.basename(DIR) + '/';
...
// son estas páginas.
const TRIP = 'China';
const TRIP_LONG = 'China, oct–dic 2027.';
...
<meta property="og:site_name" content="${esc(TRIP)} 2027">
...
(async () => {
  ...
  if (check) {
    ...
    console.error('  corré: node ' + path.basename(DIR) + '/scripts/build_dias.js');
    process.exit(1);
  }

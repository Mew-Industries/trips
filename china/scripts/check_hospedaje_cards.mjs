...
// y después:
//   node china/scripts/check_hospedaje_cards.mjs [baseUrl]
import pw from '/usr/lib/node_modules/agent-browser/node_modules/playwright-core/index.js';
const { chromium } = pw;
const ROOT = process.argv[2] || 'http://127.0.0.1:8611/china/';
...
// China no tiene vista compartida: se chequea solo la app principal.
for (const [app, base] of [['principal', ROOT]]) {
  for (const W of [1280, 390]) {
    ...
    for (const c of cards.filter(c => c.hasLodging)) {
      ...
      // Sin reserva no hay fotos del listing: 0 zonas es legítimo; lo prohibido es tener DOS.
      check(`${tag} · ${c.id} · a lo sumo una zona de imagen`, c.zonas <= 1, `${c.zonas} zonas`);
    ...
    for (const [i, m] of metas.entries()) {
      ...
      check(`${tag} · resumen · tarjeta ${i + 1} · a lo sumo una zona de imagen`, m.zonas <= 1, `${m.zonas} zonas`);
    ...
    if (carrusel) {
      ...
      // China hoy no tiene ningún hospedaje con fotos: sin carrusel no hay nada que
      // ejercitar. Cuando se cargue la primera reserva con `imgs`, esto vuelve a correr.
      console.log(`skip ${tag} · resumen · sin carrusel (ningún hospedaje con fotos todavía)`);
    }
